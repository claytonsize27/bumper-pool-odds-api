...
def fetch_player_names():
    ...
    try:
        ...
        # Check for HTTP errors
        if response.status_code != 200:
            logger.error(f"Error fetching players: {response.json()}")
            return {}
    ...
    try:
        response = supabase.table("players").select("id, name").execute()
        if response.error:
            logger.error(f"Error fetching players: {response.error}")
            return {}
        return {player["id"]: player["name"] for player in response.data}
    except Exception as e:
        logger.error(f"Failed to fetch player names: {e}", exc_info=True)
...
def implied_odds(p):
    try:
        ...
        logger.error(f"Failed to calculate implied odds: {e}", exc_info=True)
        return "Error"
...
def calculate_odds(player_a_id, player_b_id):
    try:
        ...
        if total_games == 0:
            logger.info(f"No completed games between {player_a_name} and {player_b_name}")
            return {}
        ...
        logger.error(f"Failed to calculate odds for {player_a_id} vs {player_b_id}: {e}", exc_info=True)
        return {}
